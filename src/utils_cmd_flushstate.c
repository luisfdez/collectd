...
cmd_status_t cmd_parse_flushstate(size_t argc, char **argv, cmd_flushstate_t *ret_flushstate,
  ...
  status = parse_identifier(
      argv[0], &ret_flushstate->identifier.host, &ret_flushstate->identifier.plugin,
      &ret_flushstate->identifier.plugin_instance, &ret_flushstate->identifier.type,
      &ret_flushstate->identifier.type_instance, opts->identifier_default_host);
  if (status != 0) {
    DEBUG("cmd_parse_flushstate: Cannot parse identifier `%s'.", identifier_copy);
...
cmd_status_t cmd_handle_flushstate(FILE *fh, char *buffer) {
  ...
  if (cmd.cmd.flushstate.identifier.plugin_instance != NULL)
      sstrncpy (vl.plugin_instance, cmd.cmd.flushstate.identifier.plugin_instance, sizeof (vl.plugin_instance));
  sstrncpy (vl.type, cmd.cmd.flushstate.identifier.type, sizeof (vl.type));
  if (cmd.cmd.flushstate.identifier.type_instance != NULL)
      sstrncpy (vl.type_instance, cmd.cmd.flushstate.identifier.type_instance, sizeof (vl.type_instance));
