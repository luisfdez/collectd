...
cmd_status_t cmd_parse_evalstate(size_t argc, char **argv, cmd_evalstate_t *ret_evalstate,
  ...
  status = parse_identifier(
      argv[0], &ret_evalstate->identifier.host, &ret_evalstate->identifier.plugin,
...
cmd_status_t cmd_handle_evalstate(FILE *fh, char *buffer) {
  ...
  DEBUG("utils_cmd_evalstate: cmd_handle_evalstate (fh = %p, buffer = %s);", (void *)fh,
        buffer);
  ...
  if (cmd.cmd.evalstate.identifier.plugin_instance != NULL)
      sstrncpy (vl.plugin_instance, cmd.cmd.evalstate.identifier.plugin_instance, sizeof (vl.plugin_instance));
  sstrncpy (vl.type, cmd.cmd.evalstate.identifier.type, sizeof (vl.type));
  if (cmd.cmd.evalstate.identifier.type_instance != NULL)
      sstrncpy (vl.type_instance, cmd.cmd.evalstate.identifier.type_instance, sizeof (vl.type_instance));

  if (plugin_write("threshold", ds, &vl) < 0 ) {
    DEBUG("cmd_handle_evalstate: plugin_get_ds (%s) == NULL;", cmd.cmd.getval.identifier.type);
